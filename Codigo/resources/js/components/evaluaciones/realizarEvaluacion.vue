<template>
    <span>
        ...
        <span v-if="tipoEv === 'anualStart'"> 
            <form action="anual" method="POST" @submit.prevent="enviarAnual(provAnualData['idp'])">
                <div class="row d-flex justify-content-center">
                    <span class="h5 col-12 text-center"> <b> Proveedor: </b> {{ provAnualData["prov"] }} </span>
                    <div class="col-12 h6"> &#x2022; <b> Productos disponibles en el catálogo: </b> </div>
                    <div class="col-12">
                        <table class="table table-striped border border-info">
                            <thead class="bg-primary text-white"> 
                            ...
                            <tbody>
                                <tr v-for="(esen,index) in provAnualData['esencias']" :key="esen.cas">
                                    <td class=" p-1 m-0 pl-2"> {{ esen["cas"] }} </td>
                                    <td class="p-1 m-0"> {{ esen["ing"] }} </td>
                                    <td class="p-1 m-0"> Esencia {{ esen["tipo"] }} </td>
                                    <td class="text-center p-1 m-0"> 
                ...
                <div class="flex-row d-flex justify-content-center mx-4">
                    <span class="h5 col-12 text-center"> 
                        <b> Criterio de Evaluación: </b> {{ formAnual[0]["nombre"] }} <b> 
                        ({{ formAnual[0]["peso"] }} %) </b> 
                    </span>
                </div>
                <hr class="py-0 my-0 mb-2">
                <div class="row d-flex justify-content-center mx-4">
                    <table class="table table-striped mx-4 col-8">
                        <thead class="text-center">
                            <th scope="col" class="bg-primary text-white border border-info"> Ubicación / Sede Principal </th>
                            <th scope="col" style="background-color: #E4E4E4" class="border border-info"> {{ provAnualData["pais"] }} </th>
                        </thead>
                    </table>
                </div>
                <div class="flex-row d-flex justify-content-center mx-4 align-items-center">
                    <span class="mr-2" style="font-size: 16px"> <b> Puntaje: </b> </span> 
                    <input type="number" name="anualUbic" :min="escala.ri" :max="escala.rf" class="form-control w-25" 
                    id="anualUbic" v-model.number="provAnualRes[0]" :class="anualUbicErr">
                    <span class="ml-2" style="font-size: 16px"> 
                        <b> (de {{escala.ri}} a {{escala.rf}}) </b> 
                    </span>
                </div>
                <div class="flex-row d-flex justify-content-center">
                    <span v-if="anualUbicErr === 'is-invalid'"> 
                        <small class="text-danger"> <b> El puntaje está fuera de la escala. </b> </small> 
                    </span>
                </div>
                <hr class="py-0 my-2">
                <div class="flex-row d-flex justify-content-center mx-4">
                    <span class="h5 col-12 text-center"> 
                        <b> Criterio de Evaluación: </b> {{ formAnual[1]["nombre"] }} <b> 
                        ({{ formAnual[1]["peso"] }} %) </b> 
                    </span>
                </div>
                <hr class="py-0 my-2">
                <div class="row d-flex justify-content-center mx-4">
                    <table class="table table-striped border border-info mx-4">
                        ...
                        <tbody>
                            <tr v-for="pago in provAnualData['pagos']" :key="pago.id">
                                <td> {{ pago["tipo"] }} </td> 
                                <td v-if="pago['numc'] != null" class="text-center"> 
                                    <span v-if="pago['numc'] > 1"> {{ pago["numc"] }} cuotas </span>
                                    <span v-else> 1 cuota </span>
                ...
                <div class="flex-row d-flex justify-content-center mx-4 align-items-center">
                    <span class="mr-2" style="font-size: 16px"> <b> Puntaje: </b> </span> 
                    <input type="number" name="anualPago" :min="escala.ri" :max="escala.rf" class="form-control w-25" 
                    id="anualPago" v-model.number="provAnualRes[1]" :class="anualPagoErr">
                    <span class="ml-2" style="font-size: 16px"> 
                        <b> (de {{escala.ri}} a {{escala.rf}}) </b> 
                    </span>
                </div>
                <div class="flex-row d-flex justify-content-center">
                    <span v-if="anualPagoErr === 'is-invalid'"> 
                        <small class="text-danger"> <b> El puntaje está fuera de la escala. </b> </small> 
                    </span>
                </div>
                <hr class="py-0 my-2">
                <div class="flex-row d-flex justify-content-center mx-4">
                    <span class="h5 col-12 text-center"> 
                        <b> Criterio de Evaluación: </b> {{ formAnual[2]["nombre"] }} <b> 
                        ({{ formAnual[2]["peso"] }} %) </b> 
                    </span> 
                </div>
                <hr class="py-0 my-2">
                <div class="row d-flex justify-content-center mx-4">
                    <table class="table table-striped border border-info mx-4">
                        ...
                        <tbody>
                            <tr v-for="(envio,index) in provAnualData['envios']" :key="envio.id_envio">
                                <td> {{ envio["pais"] }} </td>
                                <td> {{ envio["tipo"] }} </td>
                                <td class="text-right" v-if="envio['duracion'] > 1"> {{ envio["duracion"]}} días </td>
                                <td class="text-right" v-else> 1 día </td>
                ...
                <div class="flex-row d-flex justify-content-center mx-4 align-items-center">
                    <span class="mr-2" style="font-size: 16px"> <b> Puntaje: </b> </span> 
                    <input type="number" name="anualEnvio" :min="escala.ri" :max="escala.rf" class="form-control w-25" 
                    id="anualEnvio" v-model.number="provAnualRes[2]" :class="anualEnvioErr">
                    <span class="ml-2" style="font-size: 16px"> 
                        <b> (de {{escala.ri}} a {{escala.rf}}) </b> 
                    </span>
                </div>
                <div class="flex-row d-flex justify-content-center">
                    <span v-if="anualEnvioErr === 'is-invalid'"> 
                        <small class="text-danger"> <b> El puntaje está fuera de la escala. </b> </small> 
                    </span>
                </div>
                <hr class="py-0 my-2">
                <div class="flex-row d-flex justify-content-center mx-4">
                    <span class="h5 col-12 text-center"> 
                        <b> Criterio de Evaluación: </b> {{ formAnual[3]["nombre"] }} <b> 
                        ({{ formAnual[3]["peso"] }} %) </b> 
                    </span> 
                </div>
                <hr class="py-0 my-2">
                <div class="col-12 justify-content-center d-flex flex-row align-items-center h5 mt-4"> 
                    ...
                    <span class="ml-2"> <b> Pedidos cumplidos: </b> 
                ...
                <div class="col-12 justify-content-center d-flex flex-row align-items-center h5 mb-4"> 
                    ...
                    <span class="ml-2"> <b> Pedidos rechazados: </b> 
                        {{ provAnualData['pedidosRec'] }}
                    </span>
                </div>
                <div class="flex-row d-flex justify-content-center mx-4 align-items-center">
                    <span class="mr-2" style="font-size: 16px"> <b> Puntaje: </b> </span> 
                    <input type="number" name="anualCump" :min="escala.ri" :max="escala.rf" class="form-control w-25" 
                    id="anualCump" v-model.number="provAnualRes[3]" :class="anualCumpErr">
                    <span class="ml-2" style="font-size: 16px"> 
                        <b> (de {{escala.ri}} a {{escala.rf}}) </b> 
                    </span>
                </div>
                ...
                <div class="row d-flex justify-content-center mt-2">
                    <button type="submit" class="btn btn-primary" :class="anualFinalizarErr" :disabled="anualFinalizarDis"> 
                        <img src="/img/iconos/check_white.svg" alt="" width="24" class="mb-1">
                        <span class="ml-2"> Finalizar Evaluación </span>
                    </button>
                </div>
        ...
        <span v-if="tipoEv === 'resultadoAnual'">
            ...
            <div class="row h5 text-left ml-4 pl-4"> 
                <li> <b class="mr-1"> Fecha de Apertura: </b>
                {{ resultadoAnual["fechaA"] }} </li>
            </div>
            <div class="row h5 text-left ml-4 pl-4"> 
                <li> <b class="mr-1"> Fecha de Renovación Actual: </b> 
                {{ resultadoAnual["fechaR"] }} 
                (renovado {{ resultadoAnual["renov"] }} 
                <span v-if="parseInt(resultadoAnual['renov']) == 1"> vez </span>
                <span> veces </span> )</li>
            </div>
            <div class="row h5 text-left ml-4 pl-4"> 
                <li> <b class="mr-1"> Expira en: </b> 
                {{ resultadoAnual["exp"] }} 
...
export default {
    ...
    data() {
        return {
            ...
            provAnualRes: [0,0,0,0], /*arreglo que contiene los resultados que se van asignando
