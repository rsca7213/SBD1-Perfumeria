<template>
  <span>
    ...
    <span v-if="tipoEv === ''">
      <div class="row d-flex justify-content-center">
        ...
        <button
          ...
          @click="evInicial"
          :disabled="btnEvInicDis"
        >Inicial</button>
        <button
          class="btn btn-small btn-primary mx-4"
          @click="evAnual"
          :disabled="btnEvAnualDis"
        >Anual</button>
      </div>
      <hr />
      <div class="row d-flex justify-content-center">
    ...
    <span v-if="tipoEv === 'inicial'">
      ...
      <hr />
      <span class="h5">
        <b>Seleccionar proveedor a evaluar:</b>
      </span>
      <hr />
      <div class="row mx-4 p-2 border border-info" v-for="prov in provsInic" :key="prov.idp">
        <span class="col-12 h5">
          <b>Proveedor:</b>
          {{ prov["prov"] }}
        </span>
        <span class="col-6">
          &#x2022;
          <b>Fecha de inicio membresia IFRA:</b>
          {{ prov["memb"] }}
        </span>
        <span class="col-6">
          &#x2022;
          <b>Tipo de membresia IFRA:</b>
          {{ prov["tipo"] }}
        </span>
        <span class="col-6">
          &#x2022;
    ...
    <span v-if="tipoEv === 'anual'">
      ...
      <div class="row mx-4 p-2 border border-info" v-for="(prov,index) in provsAnual" :key="index">
        ...
        <span class="col-6">
          &#x2022;
          <b>Fecha de apertura del contrato:</b>
          {{ prov["fecha"] }}
        </span>
        <span class="col-6">
          &#x2022;
        ...
        <span class="col-12">
          &#x2022;
          <b>Fecha de renovación más reciente:</b>
          <span v-if="parseInt(prov['renov']) === 0">N/A</span>
          <span v-else>
            {{ prov["fechaR"] }} ( renovado {{ prov["renov"]}}
            <span v-if="prov['renov'] == 1">vez</span>
            <span v-else>veces</span>
            )
          </span>
        </span>
        <div class="col-12 row d-flex justify-content-center mt-2">
          <button class="btn btn-primary" @click="dataAnual(prov['idp'],prov['fecha_ap_ts'])">
            <img src="/img/iconos/check_white.svg" alt="evaluar" width="24" class="mb-1" />
            <span class="ml-2">Evaluar</span>
          </button>
        </div>
      ...
      <div
        class="mx-4 h5 text-center"
        v-if="provsAnual.length === 0"
      >Aun no tiene proveedores cuyos contratos esten a 1 mes de expirar.</div>
    </span>

    <!-- Proceso de evaluacion inicial -->
    <span v-if="tipoEv === 'inicialStart'">
      <form action="inicial" method="POST" @submit.prevent="enviarInic(provInicData['idp'])">
        <div class="row d-flex justify-content-center">
          ...
          <div class="col-12 h6">
            &#x2022;
            <b>Productos disponibles en el catálogo:</b>
          </div>
          <div class="col-12">
                ...
                <tr v-for="(esen,index) in provInicData['esencias']" :key="esen.cas">
                  ...
                  <td class="text-center p-1 m-0">
                    <img
                      ...
                      width="24"
                      class="iconobtn"
                      @click="rellenarModalDetProducto('e',index)"
                      data-toggle="modal"
                ...
                <tr v-for="(otro,index) in provInicData['otros']" :key="otro.cas">
                  ...
                  <td class="text-center p-1 m-0">
                    <img
                      ...
                      width="24"
                      class="iconobtn"
                      @click="rellenarModalDetProducto('o',index)"
                      data-toggle="modal"
                      data-target="#modalInicDetProducto"
                    />
                  </td>
        ...
        <div class="row d-flex justify-content-center mx-4">
          <table class="table table-striped mx-4 col-8">
            <thead class="text-center">
              <th
                scope="col"
                class="bg-primary text-white border border-info"
              >Ubicación / Sede Principal</th>
              <th
                scope="col"
                style="background-color: #E4E4E4"
                class="border border-info"
              >{{ provInicData["pais"] }}</th>
            </thead>
          </table>
        </div>
        ...
        <div class="flex-row d-flex justify-content-center">
          <span v-if="inicUbicErr === 'is-invalid'">
            <small class="text-danger">
              <b>El puntaje está fuera de la escala.</b>
            </small>
          </span>
        </div>
        <hr class="py-0 my-2" />
              ...
              <tr v-for="(envio,index) in provInicData['envios']" :key="envio.id_envio">
                ...
                <td class="text-center">
                  <img
                    ...
                    width="24"
                    class="iconobtn"
                    @click="rellenarModalDetEnvio(index)"
                    data-toggle="modal"
        ...
        <div class="row d-flex justify-content-center mt-2">
          <button
            ...
            :class="inicFinalizarErr"
            :disabled="inicFinalizarDis"
          >
    ...
    <!-- Proceso de evaluacion anual -->
    <span v-if="tipoEv === 'anualStart'">
      <form
        action="anual"
        method="POST"
        @submit.prevent="enviarAnual(provAnualData['idp'],provAnualData['fecha_ap_ts'])"
      >
        <div class="row d-flex justify-content-center">
          <span class="h5 col-12 text-center">
            <b>Proveedor:</b>
        ...
        <div class="col-12 justify-content-center d-flex flex-row align-items-center h5 mt-4">
          ...
          <span class="ml-2">
            ...
            {{ provAnualData['pedidosCump'] }}
          </span>
        </div>
        <div class="col-12 justify-content-center d-flex flex-row align-items-center h5 mb-4">
          <img src="/img/iconos/cancel_red.svg" width="24" />
          <span class="ml-2">
            <b>Pedidos rechazados:</b>
            {{ provAnualData['pedidosRec'] }}
          </span>
        </div>
        <div class="flex-row d-flex justify-content-center mx-4 align-items-center">
          <span class="mr-2" style="font-size: 16px">
        ...
        <div class="flex-row d-flex justify-content-center">
          <span v-if="anualCumpErr === 'is-invalid'">
            <small class="text-danger">
              <b>El puntaje está fuera de la escala.</b>
            </small>
          </span>
        ...
        <hr class="py-0 my-2" />
        <input type="hidden" name="_token" :value="this.csrf" />
        <input type="hidden" name="idp" :value="provAnualData['idp']" />
        <input type="hidden" name="fecha_ap_ts" :value="provAnualData['fecha_ap_ts']" />
        <div class="row d-flex justify-content-center mt-2">
          <button
            type="submit"
            class="btn btn-primary"
            :class="anualFinalizarErr"
            :disabled="anualFinalizarDis"
          >
            <img src="/img/iconos/check_white.svg" alt width="24" class="mb-1" />
            <span class="ml-2">Finalizar Evaluación</span>
          </button>
    ...
    <div
      class="modal fade"
      id="modalInicDetEnvio"
      tabindex="-1"
      role="dialog"
      aria-hidden="true"
      v-if="tipoEv === 'inicialStart'"
    >
    ...
    >
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content" style="background-color: #F5F5F5">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">
              <b>Presentaciones de Producto</b>
            </h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          ...
          <div class="modal-body h5 text-center">
            <span v-if="provInicData['esencias'] != [] && indexProducto[0] === 'e'">
              <h5>
                <b>{{ provInicData['esencias'][indexProducto[1]]["ing"] }}</b>
              </h5>
            </span>
            <span v-if="provInicData['otros'] != [] && indexProducto[0] === 'o'">
              <h5>
                <b>{{ provInicData['otros'][indexProducto[1]]["ing"] }}</b>
              </h5>
            </span>
            <table class="table table-striped border border-info">
              <thead class="bg-primary text-white">
              ...
              <tbody v-if="provInicData['esencias'] != [] && indexProducto[0] === 'e'">
                <tr
                  v-for="pres in provInicData['esencias'][indexProducto[1]]['pres']"
                  :key="pres.id"
                >
    ...
    <span v-if="tipoEv === 'resultadoInic'">
      ...
      <div class="row h5 justify-content-center">
        <b>Criterio de Exito:</b>
        <span class="ml-1">{{ resultadoInicial["exito"] }} %</span>
      </div>
      <div class="row h5 justify-content-center">
        <b>Resultado:</b>
      ...
      <span
        v-if="parseFloat(resultadoInicial['res']) >= parseFloat(resultadoInicial['exito'])"
        class="mt-4"
      >
        ...
        <div class="row h6 mt-4 justify-content-center">
          <b>¿Desea generar un contrato con el proveedor?</b>
        </div>
      ...
      <span v-else>
        ...
        <div class="row justify-content-center">
          <a href="realizar" class="btn btn-primary">
            <img src="/img/iconos/evaluation.svg" width="24" class="mb-1" />
            <span class="ml-2">Realizar otra evaluacion</span>
          </a>
        </div>
      </span>
    </span>
    ...
    <span v-if="tipoEv === 'resultadoAnual'">
      ...
      <div class="row h5 justify-content-center">
        <b>Criterio de Exito:</b>
        <span class="ml-1">{{ resultadoAnual["exito"] }} %</span>
      </div>
      <div class="row h5 justify-content-center">
        <b>Resultado:</b>
      ...
      <div class="row h5 text-left ml-4 pl-4">
        <li>
          <b class="mr-1">Fecha de Apertura:</b>
          {{ resultadoAnual["fechaA"] }}
        </li>
      </div>
      <div class="row h5 text-left ml-4 pl-4" v-if="resultadoAnual['fechaR'] != null">
        <li>
          <b class="mr-1">Fecha de Renovación Actual:</b>
          {{ resultadoAnual["fechaR"] }}
          ( renovado {{ resultadoAnual["renov"] }}
          <span
      ...
      <div class="row h5 text-left ml-4 pl-4" v-else>
        <li>
          <b class="mr-1">Fecha de Renovación Actual:</b> N/A
        </li>
      </div>
      <div class="row h5 text-left ml-4 pl-4">
        <li>
      ...
      <div class="row h5 text-left ml-4 pl-4">
        <li>
          <b>{{ resultadoAnual["ex"] }}</b>
        </li>
      </div>
      <hr />
      <span
        v-if="parseFloat(resultadoAnual['res']) >= parseFloat(resultadoAnual['exito'])"
        class="mt-4"
      >
        <div class="row h4 justify-content-center">
          <img src="/img/iconos/check_green.svg" width="32" />
...
export default {
  ...
  data() {
    return {
      ...
      tipoEv:
        "wait" /*encargada de determinar el tipo de evaluacion a realizar
      ...
      provAnualData: [] /*arreglo que contiene la data que devuelve laravel
      ...
      resultadoInicial: [] /* Variable que almacena los resultados que devuelve laravel
  ...
  created() {
    ...
    axios
      ...
      .then((response) => {
        console.log(
          "%cAxios: Exito en busqueda de formulas y escala!",
          "color: lightgreen"
        );
        ...
        if (this.escala.length === 0) {
          ...
          this.provAnualRes[0] = this.escala["ri"];
        }
      ...
      .catch((errors) => {
        console.log(
          "%cAxios: Error buscando formulas y escala!",
          "color: #FFCCCB"
        );
      });
  },
  ...
  methods: {
    ...
    evInicial() {
      console.log(
        "%cAxios: Get! Buscando proveedores para ev inicial",
        "color: lightblue"
      );
      axios
        .get("inicial")
    ...
    evAnual() {
      console.log(
        "%cAxios: Get! Buscando proveedores para ev anual",
        "color: lightblue"
      );
      axios
        .get("anual")
    ...
    dataInicial(idp) {
      console.log(
        "%cAxios: Get! Buscando data para ev inicial",
        "color: lightblue"
      );
      axios
        .get("data/inicial/" + idp)
        .then((response) => {
          console.log("%cAxios: Success!", "color: lightgreen");
    ...
    /* Funcion AXIOS que le pide a laravel toda la data necesaria para
           ...
           del cual necesita data */
    dataAnual(idp, fecha_ap_ts) {
      console.log(
        "%cAxios: Get! Buscando data para ev anual",
        "color: lightblue"
      );
      axios
        .get("data/anual/" + idp + "/" + fecha_ap_ts)
        .then((response) => {
          console.log("%cAxios: Success!", "color: lightgreen");
          this.provAnualData = response.data[0];
          this.tipoEv = "anualStart";
        })
        .catch((errors) => {
          console.log("%cAxios: Error!", "color: #FFCCCB");
          console.log(errors);
    ...
    enviarInic(idp) {
      console.log(
        "%cAxios: Enviando resultados de ev. inicial",
        "color: lightblue"
      );
      axios
        .post("inicial", {
        ...
        .then((response) => {
          ...
          this.generarLink =
            "/productor/" +
            this.resultadoInicial["id_prod"] +
            "/contratos/generar/" +
            this.resultadoInicial["id_prov"];
        })
        .catch((errors) => {
          console.log("%cAxios: Error!", "color: #FFCCCB");
          console.log(errors);
    ...
    enviarAnual(idp, fecha_ap_ts) {
      console.log(
        "%cAxios: Enviando resultados de ev. anual",
        "color: lightblue"
      );
      axios
        .post("anual", {
  ...
  computed: {
    ...
    inicUbicErr() {
      if (
        this.provInicRes[0] < parseInt(this.escala["ri"]) ||
        this.provInicRes[0] > parseInt(this.escala["rf"])
      ) {
        return "is-invalid";
      } else return "";
    },
    ...
    inicPagoErr() {
      if (
        this.provInicRes[1] < parseInt(this.escala["ri"]) ||
        this.provInicRes[1] > parseInt(this.escala["rf"])
      ) {
        return "is-invalid";
      } else return "";
    },
    ...
    inicEnvioErr() {
      if (
        this.provInicRes[2] < parseInt(this.escala["ri"]) ||
        this.provInicRes[2] > parseInt(this.escala["rf"])
      ) {
        return "is-invalid";
      } else return "";
    },
    ...
    anualCumpErr() {
      if (
        this.provAnualRes[0] < parseInt(this.escala["ri"]) ||
        this.provAnualRes[0] > parseInt(this.escala["rf"])
      ) {
        return "is-invalid";
      } else return "";
    },
